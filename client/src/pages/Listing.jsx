...
export default function Listing() {
  ...
  const [mapCenter, setMapCenter] = useState([]); // Default center
  ...
  useEffect(() => {
    ...
    fetchCoordinates();
    // getCoordinatesForListing(listing).then((coords) =>
    //   setMapCenter([coords.lat, coords.lng])
    // );
  }, [listing]);
        ...
        <div className="flex justify-center items-center h-screen">
          <div className="flex items-center text-center">
            ...
            <div className="ml-6">
              ...
              <button
                className="flex flex-col items-center justify-center p-3 text-blue-700 border border-blue-700 rounded uppercase hover:shadow-lg disabled:opacity-80 w-50% mx-auto"
                onClick={() => Navigate("/")}
              >
                Return to Homepage
              </button>
                  ...
                  >
                    ...
                    {mapCenter && (
                      <Marker position={mapCenter} icon={customIcon}>
                        <Popup>
                          The address of the property is: {listing.address}
                        </Popup>
                      </Marker>
                    )}
                  </MapContainer>
              ...
              <div className="flex flex-col md:w-5/12">
                <div className="flex flex-col justify-between max-w-4xl mx-auto p-3 my-7 gap-4">
                  ...
                  <div className="flex gap-4 justify-center">
                    {listing.offer && (
                      <p className="bg-green-900 w-full max-w-[300px] text-white text-center p-1.5 rounded-md flex justify-center items-center">
                        ...
                        ).toLocaleString("en-US")} is applied`}
                      </p>
                    )}
                    {listing.offer && (
                      <p className="bg-green-900 w-full max-w-[300px] text-white text-center p-1.5 rounded-md flex justify-center items-center">
                        {`Original price is $${listing.regularPrice.toLocaleString(
                          "en-US"
                        )}`}
                      </p>
                    )}
                  ...
                  <div className="text-slate-800 items-stretch">
                    <p className="font-semibold mb-2 mt-5 text-black text-xl">
